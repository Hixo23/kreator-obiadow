...
export const SingleComment = ({ comment }: { comment: MappedComment }) => {
  ...
  return (
    <div className="relative mb-4 flex w-full items-start space-x-4">
      <Avatar>
        <AvatarImage
          width={64}
      ...
      <div>
        ...
        {isEditing ? (
          <EditComment comment={comment} setIsOpen={setIsEditing} />
        ) : (
          <p className="text-muted-foreground">{comment.content}</p>
        )}
      </div>
      {user?.username === comment.author.username && !isEditing ? (
        <CommentControls setIsEditing={setIsEditing} comment={comment}>
